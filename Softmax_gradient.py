...
if __name__ == "__main__":
    ...
    patchsize = 28
    inputSize = patchsize*patchsize      # number of input units                     # looks like a lower-case "p" in the lecture notes).
    decayWeight = 1e-4   # weight decay parameter
    ...
    intercept = np.ones(patches.shape[0])
    intercept = intercept.reshape(patches.shape[0],1)
    patches = np.concatenate((intercept, patches),axis=1)
    print(patches.shape)

    # #======================================================================
    cost,grad = softmaxCost(theta, numClasses, inputSize, labels, decayWeight, patches)
    #
    # ##======================================================================
    ...
    numgrad = computeNumericalGradient(lambda x: softmaxCost(x, numClasses,
                                                            inputSize, labels, decayWeight,patches)[0],theta)
    ...
    #theta = initializeSoftmaxParameters(numClasses,inputSize)
    ...
    #
    #
    # test,label_test = sampleIMAGES(patchsize,"testing")
    # intercept = np.ones(test.shape[0])
    # intercept = intercept.reshape(test.shape[0],1)
