...
def sparseAutoencoderCost(theta,visibleSize,hiddenSize,decayWeight,sparsityParam,beta,data):
    ...
    storeActivation = np.zeros(hiddenSize)
    ...
    for i in range(0,data.shape[0]):
        z2 = np.dot(data[i,:], W1)+ b1
        # 1*64*64*25 = 1*25
        a2 = sigmoid(z2)
        storeActivation = storeActivation + a2
    ...
    for i in range(0,data.shape[0]):
        ...
        W1grad = W1grad + dev_w1
        W2grad = W2grad + dev_w2
        b1grad = b1grad + delta_2
        b2grad = b2grad + delta_3
        #computing cost
        cost = cost + 0.5*np.sum((data[i,:]-a3)*(data[i,:]-a3))

    W1grad = W1grad/data.shape[0] + decayWeight*W1
    W1grad = np.array(W1grad)
    W2grad = W2grad/data.shape[0] + decayWeight*W2
    W2grad = np.array(W2grad)
    b1grad = b1grad/data.shape[0]
    b2grad = b2grad/data.shape[0]
    ...
    cost = cost/data.shape[0] + 0.5*decayWeight*(np.sum(W1*W1)+np.sum(W2*W2))
    #print(cost)
    k = 0
    for i in range(0, hiddenSize):
        k = k + sparsityParam*np.log(sparsityParam/rho[i])+(1 - sparsityParam)*np.log((1 - sparsityParam)/(1 - rho[i]))
        #print(k)
    cost = cost + beta*k
    #print(cost)
    #-------------------------------------------------------------------
